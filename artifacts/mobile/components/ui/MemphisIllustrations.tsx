 ...
 *
 * Built with:
 *   • Reanimated v3/v4  — UI-thread worklets
 *   • react-native-svg v15
 *
 * ⚠  All animated transforms use individual SVG props (translateX, translateY,
 *    scale, scaleX, rotation, originX, originY) instead of transform strings.
 *    Passing transform as a string via useAnimatedProps crashes on Android new
 *    arch: "java.lang.String cannot be cast to ReadableArray".
 */
...
export function ChatIllustration({ size = 160 }: { size?: number }) {
  ...
  // ✅ Use translateY (number) instead of transform string
  ...
  // ✅ scale with static originX/Y on the element
  ...
  return (
    <Svg width={size} height={size} viewBox="0 0 200 200">
      ...
      {/* originX/Y are static props; only scale is animated */}
      <AnimG originX={55} originY={28} animatedProps={pBub}>
...
export function SecurityIllustration({ size = 160 }: { size?: number }) {
  ...
  // ✅ translateY instead of transform string
  const pBob = useAnimatedProps(() => ({ translateY: bobP.value }));
  // ✅ scale + opacity; originX/Y on element
  ...
  // ✅ rotation (degrees) instead of transform string; originX/Y on element
  ...
  return (
    <Svg width={size} height={size} viewBox="0 0 200 200">
      ...
      {/* originX/Y = shield centre (152, 90) */}
...
export function DiscoverIllustration({ size = 160 }: { size?: number }) {
  ...
  // ✅ All individual props — no transform strings
  const pBob = useAnimatedProps(() => ({ translateY: bobP.value }));
...
export function WalletIllustration({ size = 160 }: { size?: number }) {
  ...
  // Coin flip: scaleX −1 → +1 → −1 (3-D spin illusion)
  ...
  // ✅ translateY and scaleX instead of transform strings
  const pBobL = useAnimatedProps(() => ({ translateY: bobL.value }));
  const pBobR = useAnimatedProps(() => ({ translateY: bobR.value }));
  // scaleX with originX/Y as static props on the element
...
export function CommunityIllustration({ size = 160 }: { size?: number }) {
  ...
  // Heart beat scaled around its centre (100, 42)
  ...
  const pBobL = useAnimatedProps(() => ({ translateY: bobL.value }));
  const pBobR = useAnimatedProps(() => ({ translateY: bobR.value }));
  // scale with originX/Y as static props on element
